...
# this code is adapted from https://github.com/hubertjb/dynamic-spatial-filtering/blob/master/datasets.py#L196
# set here your local path to the data
PC18_DIR = op.join("D:\\", '../data', 'pc18')
...
class PC18(BaseConcatDataset):
    ...
    def __init__(self, subject_ids=None, path=None, load_eeg_only=True,
                 preproc=None, windower=None, n_jobs=1, save_dir=None):
        ...
        print(subject_ids)
        paths = fetch_pc18_data(subject_ids, path=PC18_DIR)
        ...
        if n_jobs == 1:
            all_base_ds = [self._load_raw(
                ...
                preproc=preproc, windower=windower, save_dir=save_dir)
    ...
    def _load_raw(self, subj_nb, raw_fname, arousal_fname, load_eeg_only,
        ...
        if preproc is not None:
            print(subj_nb)
            l = (subj_nb - 989) / 100
            _preprocess(out, None, preprocessors=preproc)
